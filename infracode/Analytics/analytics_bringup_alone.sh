...
# read -p "Please enter your instance type (e.g. t2.large): " INST_TYPE
# read -p "Please enter the path of your key file: " KEY_FILE_PATH

# wget the python file to verify key
# wget ....

python3 verify_key.py Grp5child_$NUM_CHILD

echo "Beginning cluster bring up"
flintrock --debug launch GRP5$CLUSTER_NAME --num-slaves $NUM_CHILD --spark-version 3.0.1 --hdfs-version 3.2.1 --ec2-key-name Grp5child_$NUM_CHILD --ec2-identity-file Grp5child_$NUM_CHILD.pem --ec2-ami ami-04d29b6f966df1537 --ec2-instance-type t2.xlarge --ec2-user ec2-user --install-hdfs --install-spark
